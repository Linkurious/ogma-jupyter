...
class TestLocalOgmaFallback:
    ...
    def test_configured_file_path_takes_priority(self, tmp_path, monkeypatch):
    ...
    def test_missing_configured_path_warns_and_falls_back(self, tmp_path, monkeypatch):
        ...
        assert result.name in ("ogma.umd.cjs", "ogma.umd.js")
...
class TestLicenseKeyResolution:
    ...
    def test_explicit_arg_wins(self, monkeypatch):
        monkeypatch.setattr(_config, "_license_key", "configured")
        monkeypatch.setenv("OGMA_LICENSE_KEY", "env")
        assert _config.get_license_key("explicit") == "explicit"

    def test_configured_license_key(self, monkeypatch):
        monkeypatch.setattr(_config, "_license_key", "cfg-license")
        assert _config.get_license_key() == "cfg-license"

    def test_env_var(self, monkeypatch):
        monkeypatch.setenv("OGMA_LICENSE_KEY", "env-license")
        assert _config.get_license_key() == "env-license"

    def test_empty_when_unset(self):
        assert _config.get_license_key() == ""
...
class TestLazyBundleResolution:
    ...
    def _reset(self, monkeypatch):
        ...
        monkeypatch.setattr(_widget, "_failed_license_keys", set())

    def test_no_license_does_not_trigger_download(self, tmp_path, monkeypatch):
        with patch("ogma_jupyter.downloader.CACHE_DIR", tmp_path), \
             patch("ogma_jupyter.downloader.urllib.request.urlopen") as mock_dl:
    ...
    def test_license_triggers_download(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OGMA_LICENSE_KEY", "real-license")
