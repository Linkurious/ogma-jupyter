...
def _resolve_bundle_source() -> Optional[str]:
    ...
    2. A repo-local Ogma UMD build from ``node_modules`` (development/testing
       convenience — no network or download secret needed; never present in an
       installed distribution).
    3. A lazy download-and-assemble using a dedicated download secret
       (``og.set_license(download_secret=...)`` or the ``OGMA_DOWNLOAD_SECRET``
       env var). A bare runtime license key does *not* trigger a download here.
    ...
    # repo's examples and tests render offline without exposing a real secret.
    ...
    # Only a dedicated download secret (not a bare license key) triggers a
    # network download on render.
    secret = config.get_download_secret(allow_license_fallback=False)
    if not secret or secret in _failed_download_secrets:
        return None

    try:
        bundle_path = ensure_bundle(secret)
    except Exception as exc:  # noqa: BLE001 - surface any download/assembly issue
        _failed_download_secrets.add(secret)
        warnings.warn(
...
# Download secrets that already failed this session, so repeated widget creation
# does not retry the network for a known-bad secret.
_failed_download_secrets: set = set()
