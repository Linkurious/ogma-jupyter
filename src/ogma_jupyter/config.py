...
# Module-level path to a local Ogma UMD build — set via og.set_ogma_path() or the
# OGMA_JS_PATH env var. When configured, Ogma is loaded from this local file
# instead of being downloaded (no license key or network access required).
_ogma_path: str = ""
...
def get_ogma_path(provided_path: Optional[str] = None) -> str:
    """Resolve the path to a local Ogma UMD build, if one is configured.

    Priority: explicit argument > og.set_ogma_path() > OGMA_JS_PATH env var.
    ...
    if _ogma_path:
        return _ogma_path
    return os.environ.get("OGMA_JS_PATH", "")
